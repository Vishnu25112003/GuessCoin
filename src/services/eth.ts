...
export type WalletProvider = unknown;

export function getBrowserWallet(): WalletProvider | null {
  const w = (window as { selectedWallet?: unknown; ethereum?: unknown }).selectedWallet || (window as { selectedWallet?: unknown; ethereum?: unknown }).ethereum || null;
  return w;
}
...
export function getTokenContract(web3: Web3) {
  return new web3.eth.Contract(TokenAbi as ContractAbi, TOKEN_CONTRACT_ADDRESS);
}

export function getLogicContract(web3: Web3, logicAddress: string) {
  return new web3.eth.Contract(LogicAbi as ContractAbi, logicAddress);
}

// Read-only contracts via Infura (avoid MetaMask RPC indexing issues on view calls)
export function getTokenContractReadonly() {
  return new npInfura.eth.Contract(TokenAbi as ContractAbi, TOKEN_CONTRACT_ADDRESS);
}

export function getLogicContractReadonly(logicAddress: string) {
  return new npInfura.eth.Contract(LogicAbi as ContractAbi, logicAddress);
}
