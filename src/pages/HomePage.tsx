...
import { INFURA_HTTP_URL } from "../services/config";
import LogicAbi from "../abi/LogicCrt.json";

export default function HomePage() {
  const [wallet] = useState<string | null>(
  ...
  const load = useCallback(async () => {
    ...
    const logicCrt = new web3ro.eth.Contract(LogicAbi as ContractAbi, logic);
    const entries: GuessEntry[] = [];
    const rtdbRaw = localStorage.getItem(`rtdb:${account}`);
    const rtdb: RTDBData = rtdbRaw ? JSON.parse(rtdbRaw) : {};
    for (let i = 1; i <= 5; i++) {
      const d: ContractGuessEntry = await logicCrt.methods
        .getGuessEntry(i)
  ...
  async function onGetBalance() {
    try {
      ...
      const account = localStorage.getItem("currentAccount");
      if (!account) {
        Swal.fire("No wallet", "Connect your wallet first", "error");
        return;
      }
  ...
  async function onSyncPool() {
    try {
      ...
      if (!provider) {
        Swal.fire("No wallet provider", "Open MetaMask and retry", "error");
        return;
      }
      ...
      await sendWithFees(
        ...
        { from: account },
        {
          ...
          onReceipt: (receipt: TransactionReceipt) => {
            ...
            const ev = receipt.events?.updatedPoolData?.returnValues as { _userAddress?: string; updatedStatus?: boolean; updateStatus?: boolean } | undefined;
            if (
              ev &&
              ev._userAddress &&
              ev._userAddress.toLowerCase() === account.toLowerCase()
            ) {
      ...
      if ((e as { code?: number })?.code === 4001)
        Swal.fire("Rejected", "User rejected the transaction", "error");
      else if (isGasFeeError(e))
        Swal.fire(
          "Gas fee too low",
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
          <div className="flex items-center gap-4">
            <img
              ...
              className="w-16 h-16 rounded-full ring-2 ring-sky-100"
            />
            <div>
              <p className="text-slate-900 font-semibold text-lg">Welcome</p>
              <p className="text-slate-600 break-all text-sm">
          ...
          onValidity={async (g) => {
            try {
              ...
              if (!Number.isFinite(target) || target <= 0) {
                await Swal.fire(
                  "No target block",
                  "This row has no target block number",
                  "info",
                );
                return;
              }
              if (head < target) {
                await Swal.fire(
                  "Block not reached",
                  `Current block: ${head}. Target: ${target}.`,
                  "info",
                );
                return;
              }
              ...
              if (blk?.hash) {
                ...
                await Swal.fire(
                  "Couldn't be mined",
                  "Block not available yet. Try again shortly.",
                  "warning",
                );
              }
            } catch (e: unknown) {
              await Swal.fire(
                "Error",
                (e as Error)?.message || "Unable to check validity",
                "error",
              );
            }
          }}
