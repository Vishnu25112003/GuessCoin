...
import { getTokenContract, getTokenContractReadonly, getLogicContract } from "../services/eth";
import { isGasFeeError, sendWithFees } from "../services/tx";
import type { GuessEntry, RTDBEntry } from "../types";
...
export default function NewGuessPage() {
  ...
  function validate(): boolean {
    ...
    if (overwrite === "false")
      errs.overwrite = "Cannot submit the form with overwrite is false";
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    if (!account) {
      Swal.fire("Not logged in", "No wallet account found", "error");
      return;
    }

    // Determine paid amount like HTML/JS: 25 tokens -> 25e18
    const provider =
      (window as { selectedWallet?: unknown; ethereum?: unknown }).selectedWallet || (window as { selectedWallet?: unknown; ethereum?: unknown }).ethereum;
    const web3 = new Web3(provider as never);
    const amountWei =
      paidGuess === "true" ? web3.utils.toWei("25", "ether") : "0";
    ...
    if (paidGuess === "true") {
      ...
      if (BigInt(String(bal)) < BigInt(String(amountWei))) {
        ...
        await Swal.fire(
          "Insufficient GUESS balance",
          `You need at least 25 GUESS to submit a paid guess. Current balance: ${pretty}`,
          "error",
        );
        return;
      }
    ...
    if (paidGuess === "true") {
      try {
          ...
          await sendWithFees(
            ...
            { from: account },
            {
              onHash: () => {
                void Swal.fire(
                  "Approving",
                  "Approving 25 tokens for Logic contract…",
                  "info",
                );
              },
            },
        ...
        if (isGasFeeError(err))
          ...
          await Swal.fire(
            "Approval error",
            (err as Error)?.message || "Failed to approve tokens",
            "error",
          );
        return;
      }
    ...
    try {
      ...
      await sendWithFees(
        ...
        { from: account },
        {
          onHash: () => {
            Swal.fire(
              "Transaction sent",
              "Don't refresh. Waiting for confirmation...",
              "info",
            );
          },
        },
      ...
      if (isGasFeeError(err))
        ...
        Swal.fire(
          "Transaction error",
          (err as Error)?.message || "Failed to send transaction",
          "error",
        );
      return;
    }
    ...
    let table: Record<string, RTDBEntry> = {};
    try {
  ...
  return (
    <div>
      ...
      <div className="max-w-3xl mx-auto py-8">
        ...
        <form
          onSubmit={onSubmit}
          className={`space-y-5 ${isReadOnly ? "opacity-90" : ""}`}
        >
          <div className="grid md:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input
                className="w-full border rounded p-2"
                value={blockInc}
                onChange={(e) => setBlockInc(e.target.value)}
                readOnly={isReadOnly}
              />
              {errors.blockInc && (
            ...
            <div>
              ...
              <select
                className="w-full border rounded p-2"
                value={paidGuess}
                onChange={(e) => setPaidGuess(e.target.value as "true" | "false")}
                disabled={isReadOnly}
              >
            ...
            <div>
              ...
              <select
                className="w-full border rounded p-2"
                value={overwrite}
                onChange={(e) => setOverwrite(e.target.value as "true" | "false")}
              >
                <option value="true">true</option>
                <option value="false">false</option>
              </select>
            ...
            <div>
              ...
              <select
                className="w-full border rounded p-2"
                value={complex}
                onChange={(e) => setComplex(e.target.value as "true" | "false")}
                disabled={isReadOnly}
              >
                <option value="true">true</option>
                <option value="false">false</option>
          ...
          {/* Paid/Free notes */}
          {paidGuess === "true" && (
            <div className="p-3 bg-emerald-50 text-emerald-700 rounded border">
              Proceeding with Paid Guess.
            </div>
          )}
          {paidGuess === "false" && (
            <div className="p-3 bg-sky-50 text-sky-700 rounded border">
              Proceeding with Free Guess.
            </div>
          )}
