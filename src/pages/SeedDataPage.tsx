...
export default function SeedDataPage() {
  ...
  async function fetchGenerated() {
    ...
    if (blockDistance <= 128) {
      ...
      const [randomBlockHash] = await getRandomBlockHash(
        block.hash,
        targetBlockNumber,
      );
      if (randomBlockHash) {
  ...
  async function verifyOffChain() {
    if (!guess || !generatedHash) {
      await Swal.fire(
        "No generated hash",
        "Fetch generated hash first",
        "error",
      );
      return;
    }
    const m = await compareHexValues(
      generatedHash,
      guess.actualHash || "",
      guess.tokenSize,
    );
  ...
  return (
    <div>
      <Header
        trail={[{ label: "Home", to: "/Home" }, { label: "Get Seed Data" }]}
        onLogout={() => {
          localStorage.clear();
      ...
      <div className="max-w-5xl mx-auto px-4 py-8">
        ...
        {guess && (
          <div className="space-y-3 text-sm bg-white border rounded-lg p-4">
            <div>
              <span className="text-slate-500">Guess ID</span>:{" "}
              <span className="font-medium">{guess.guessId}</span>
            </div>
            <div>
              <span className="text-slate-500">Target Block Count</span>:{" "}
              <span className="font-medium">{guess.targetBlockNumber}</span>
            </div>
            <div>
              <span className="text-slate-500">Token Size</span>:{" "}
              <span className="font-medium">{guess.tokenSize}</span>
            </div>
            <div>
              <span className="text-slate-500">Paid Guess</span>:{" "}
              <span className="font-medium">{String(guess.paidGuess)}</span>
            </div>
            <div>
              <span className="text-slate-500">Complex</span>:{" "}
              <span className="font-medium">{String(guess.complex)}</span>
            </div>
            <div className="break-all">
              <span className="text-slate-500">Actual Hash</span>:{" "}
              <span className="font-mono">{guess.actualHash}</span>
            </div>
            <div className="break-all">
              <span className="text-slate-500">Secret Key</span>:{" "}
              <span className="font-mono">{guess.secretKey}</span>
            </div>
            <div className="break-all">
              <span className="text-slate-500">Dummy Hash</span>:{" "}
              <span className="font-mono">{guess.userHashGuess}</span>
            </div>
          </div>
        )}
        ...
        <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-3 items-end">
          ...
          <button
            ...
            className="h-11 bg-sky-700 hover:bg-sky-800 text-white rounded"
          >
