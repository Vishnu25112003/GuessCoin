  ...
  const ConnectedView = () => (
    <div className="bg-white border rounded-xl shadow-sm p-4">
      ...
      <div className="mt-4 flex gap-2">
        <button className="px-4 py-2 bg-sky-700 hover:bg-sky-800 text-white rounded" onClick={() => (window.location.href = '/session')}>Continue</button>
        <button className="px-4 py-2 border rounded hover:bg-slate-50" onClick={disconnect}>Disconnect</button>
      </div>
    </div>
  );
      ...
      <div className="space-y-2">
        {providers.map((p) => (
          <label key={p.info.uuid} className="flex items-center gap-3 p-2 border rounded-lg cursor-pointer hover:bg-slate-50">
            <img src={logos[p.info.name] || 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTpdkhaco7zP_okC9wum0TqCmJUi7vehbO6jg&s'} className="w-10 h-10 rounded" />
