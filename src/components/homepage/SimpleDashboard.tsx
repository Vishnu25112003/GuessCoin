...
import SimpleGuessRing from "./SimpleGuessRing";
...
const MainDashboard: React.FC<MainDashboardProps> = ({ guesses }) => {
  ...
  return (
    <div className="pt-16 font-mono">
      <div className="gc-geometric-bg" aria-hidden="true" />
      <div className="gc-dots-pattern" aria-hidden="true" />
      ...
      >
        <SimpleGuessRing
          ...
          onNewGuess={() => setCurrentAction("New Guess")}
          onVerify={() => setCurrentAction("Verify")}
          onCheckValidity={() => setCurrentAction("Check Validity")}
        />
      </div>
      ...
      <style>{`
        .gc-geometric-bg {
          ...
          pointer-events: none;
          background:
            radial-gradient(50% 40% at 50% 45%, rgba(59,130,246,0.12) 0%, rgba(59,130,246,0.00) 70%),
        ...
        @keyframes floatSide {
          0%, 100% { transform: rotate(12deg) translateX(0px); opacity: 0.5; }
          50% { transform: rotate(15deg) translateX(10px); opacity: 0.7; }
        }
