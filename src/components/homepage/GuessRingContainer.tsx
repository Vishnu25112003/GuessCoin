...
const readSingleGuess = useCallback((guessId: number): HashDetails | null => {
  try {
  ...
  const buildAll = useCallback((): Record<number, HashDetails> => {
  ...
  // Actions (unchanged)
  const handleNewGuessClick = useCallback((guessId: number) => {
