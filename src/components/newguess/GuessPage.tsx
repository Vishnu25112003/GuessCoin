...
interface TransactionReceipt {
  ...
  events?: {
    guessSubmitted?: {
      returnValues: {
        _guessBlockNumber?: string;
        guessBlockNumber?: string;
        2?: string;
        [key: string]: unknown;
      };
    };
...
const GuessPage: React.FC = () => {
  ...
  useEffect(() => {
    if (guessId < 0 || guessId > 255) {
      alert(
        `⚠️ Invalid Guess ID: ${guessId}\n\n` +
        `Guess ID must be between 0 and 255 due to smart contract limitations (uint8 type).\n\n` +
        `Please use a valid Guess ID.`
      );
      navigate("/dashboard");
    }
  }, [guessId, navigate]);
  ...
  const initContractInstance = async (contractType: "token" | "logic") => {
    try {
      ...
      let contractABI: typeof TOKEN_CONTRACT_ABI | typeof LOGIC_CONTRACT_ABI;
    ...
    try {
      ...
      if (paidGuess) {
        try {
          ...
          if (txError.message?.includes("Insufficient token balance")) {
            throw error;
          }
      ...
      await logicContract.methods
        ...
        .on("receipt", function (receipt: unknown) {
          ...
          const safeReceipt = convertBigIntToString(receipt) as TransactionReceipt;
...
localStorage.setItem(`guesses/${walletAddress}/${guessId}`, JSON.stringify(exportableData));
      ...
      const txError = error as TransactionError;
      console.error("Error submitting guess:", txError);
      let errorMessage = "An error occurred while submitting the form.";
      if (
        txError.message?.includes("User denied transaction signature") ||
        txError.code === 4001
      ) {
      ...
      } else if (txError.message?.includes("Logic contract")) {
        errorMessage = txError.message;
      } else if (txError.message?.includes("Wrong network")) {
        errorMessage = txError.message;
      } else if (txError.message?.includes("Token approval failed")) {
        errorMessage =
