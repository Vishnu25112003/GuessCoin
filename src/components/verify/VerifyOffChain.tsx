...
const VerifyOffChain: React.FC = () => {
  ...
  const [isVerifyOnchainEnabled, setIsVerifyOnchainEnabled] = useState(false);
  ...
  const handleFetchAndVerify = async () => {
    ...
    try {
      ...
      if (matchedTokens.length > 0) {
        ...
        // Enable verify onchain button
        setIsVerifyOnchainEnabled(true);

        showAlert(
          "success",
          `${matchedTokens.length} Match(es) Found!`,
          `Matching combinations found: (${matchedTokens.join(", ")}). You can now proceed to verify on-chain.`,
        );
      } else {
        setIsVerifyOnchainEnabled(false);
        showAlert(
          "info",
          "No Matches Found",
  ...
  const handleVerifyOnchain = () => {
    if (!isVerifyOnchainEnabled) return;

    navigate(`/verify-onchain/${guessId}`, {
      state: {
        guessId: guessId,
        actualHash: removePrefix(actualHash),
        secretHash: removePrefix(secretKey),
        targetBlockHash: removePrefix(targetBlockHash),
        fetchedHash: removePrefix(targetBlockHash),
        tokenSize,
        targetBlockNumber: targetBlockCount,
        storedGuessData: storedGuessData,
        blockDistance: blockRangeIndication?.blockDistance || 0,
        complex,
        matches: foundTokens,
        encodedMatch: localStorage.getItem("matchData")
          ? JSON.parse(localStorage.getItem("matchData")!).encodedMatch
          : "",
      },
    });
  };
    ...
    >
      ...
      <div className="max-w-7xl mx-auto mb-6 relative z-10">
        ...
        <p className="text-gray-300">
          Compare your guess with the blockchain hash to find matches
        </p>
        <p className="text-sm text-yellow-400 mt-1">
          ℹ️ This step only compares hashes. To claim rewards, proceed to On-Chain Verification.
        </p>
              ...
              <div className="space-y-4">
                ...
                <div className="flex justify-center">
                  <button
                    ...
                    disabled={!isVerifyOnchainEnabled}
                    className="w-full md:w-2/3 flex items-center justify-center gap-2 px-6 py-4 bg-gradient-to-r from-green-600 to-emerald-600 hover:from-green-700 hover:to-emerald-700 text-white rounded-lg font-semibold disabled:opacity-50 disabled:cursor-not-allowed transition-all shadow-lg text-lg"
                  >
                    <CheckCircle size={24} />
                    Verify Onchain
                  </button>
                ...
                {!isVerifyOnchainEnabled && (
                  <p className="text-center text-sm text-gray-400">
                    Click "Fetch & Verify Hash" first to find matches
                  </p>
                )}
              </div>
            </motion.section>
